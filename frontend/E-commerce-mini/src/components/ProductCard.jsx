...
export default function ProductCard({ product, onAddToCart }) {
  ...
  return (
    <div
      ...
      className="group flex flex-col bg-white rounded-[20px] overflow-hidden border border-slate-200/70 hover:-translate-y-1 hover:border-accent/30 transition-all duration-300 cursor-pointer focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2"
    >
      {/* Image Container */}
      <div className="relative aspect-[4/3] overflow-hidden bg-slate-50 p-6 flex items-center justify-center">
        ...
        <div className="absolute top-4 left-4 z-20 flex flex-col gap-2">
          {product.discount && (
            <span className="px-3 py-1 bg-white/90 backdrop-blur border border-slate-200 text-slate-700 text-xs font-bold rounded-full">
              -{product.discount}%
            </span>
          )}
          {product.isNew && (
        ...
        <div className="absolute right-4 top-4 z-20 translate-x-8 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-300 flex flex-col gap-2">
          ...
          <button onClick={stopNavigation} className="w-10 h-10 bg-white/90 backdrop-blur rounded-full flex items-center justify-center text-slate-600 hover:text-accent hover:bg-white shadow-sm transition-colors">
      ...
      <div className="p-6 flex flex-col flex-grow">
        <div className="flex items-center gap-1 text-xs font-semibold text-accent mb-2 uppercase tracking-wider">
          {product.category || "Gadget"}
        </div>

        <h3 className="text-lg font-bold text-slate-900 line-clamp-2 mb-2 group-hover:text-accent transition-colors leading-snug">
        ...
        <div className="mt-auto">
          ...
          {isOutOfStock ? (
            <button
              disabled
              className="w-full bg-slate-100 text-slate-400 font-semibold py-3.5 px-4 rounded-full cursor-not-allowed flex items-center justify-center gap-2"
            >
              Hết hàng
            </button>
          ) : (
            <button
              ...
              className="w-full bg-slate-900 hover:bg-accent text-white font-semibold py-3.5 px-4 rounded-full transition-all duration-300 transform active:scale-[0.98] flex items-center justify-center gap-2"
            >
